...
class parser
{
    ...
    //  Keep declarations generated by a metafunction - to be appended to
    //  translation unit after the metafunction invocations for a given
    //  declaration have been finished
    std::vector<std::unique_ptr<declaration_node>> generated_declarations = {};
...
public:
    ...
    {
        prepare_parse_one(tokens_, generated_tokens_);

        try {
            //  Parse one statement - we succeed if the parse succeeded,
        ...
        -> std::vector< declaration_node const* >
    {
        ...
        for (auto& decl : parse_tree->declarations)
        {
            ...
            if (decl->position().lineno >= first_line ||
                decl->position().lineno <  0 //  Generated decl by metafunction
                )
            {
...
private:
    ...
    //-----------------------------------------------------------------------
    //  Preparation for generated token parsing
    //
    //  tokens              input tokens for this section of Cpp2 source code
    //  generated_tokens    a shared place to store generated tokens
    //
    //  Sets per-parse state for the duration of the next parse_one* call.
    auto prepare_parse_one(
        std::vector<token> const& tokens_,
        stable_vector<token>&     generated_tokens_
    )
        -> void
        ...
        -> std::unique_ptr<translation_unit_node>
    {
        ...
        for (auto d = declaration(); d; d = declaration()) {
            ...
            for(auto& gd : generated_declarations) {
                n->declarations.push_back( std::move(gd) );
            }
